...
from etl_utils import env_bool, env_str
...
from timebox import get_site_clock, site_window_from_lookback, site_to_utc, to_epoch_ms
...
def main():
    ...
    try:
        ...
        site_now = datetime.now(timezone.utc)  # bạn có thể đổi sang site tz nếu muốn
        day_to = site_now.date()
        ...
        from_local, to_local = site_window_from_lookback(clk.now_site, cfg.app.lookback_h)
        ...
        for dev_name, meta in devices.items():
            ...
            from_utc_dev, to_utc_dev = compute_from_to(
                last_end, hard_from_utc, hard_to_utc,
                cfg.app.overlap_min, cfg.app.max_backfill_h
            )
            logger.info("STEP-06:Device {} → last_end={} | window {} → {}",
                        dev_name, last_end, from_utc_dev, to_utc_dev)
        ...
        for dev_name, meta in devices.items():
            ...
            last_end = get_last_event_end_ts(pg_engine, mid)
            from_utc_dev, to_utc_dev = compute_from_to(
                last_end, from_utc, to_utc, cfg.app.overlap_min, cfg.app.max_backfill_h
            )
            ...
            for probe in [cfg.domain.tags.machine_state, cfg.domain.tags.counter]:
                ...
                logger.info("Sample points for key '{}' -> {} rows | first={}", probe, len(samples), samples[0] if samples else None)
            ...
            logger.info("STEP-08A: Normalize timeseries Device {} Normalized timeline -> {} points | first={}", dev_name, len(timeline), first)
                # ...  STEP-08B
            segments = derive_state_points(timeline)
            logger.info("STEP-08B: Derived segments -> {}", len(segments))
            if segments:
                first = segments[0]
                logger.debug("First seg: start={} end={} state_id={}",
                            first["start_ts"], first["end_ts"], first["state_id"])
                # ...  STEP-08C
            segments = coalesce_segments(segments, shift_boundaries=None)
            first_segments = segments[0] if segments else None
            ...
            if segments:
                first = segments[0]
                logger.debug(
                    "First compressed segment: reason_id={} start={} end={} duration={}",
                    first["reason_id"],first["start_ts"], first["end_ts"], first["duration_sec"]
                )
            ...
            if merged:
                m0 = merged[0]
                logger.debug(
                    "After merge: first seg start={} end={} duration={} state_id={} reason_id={} po={} packaging_id={}",
                    m0["start_ts"], m0["end_ts"], m0["duration_sec"], m0.get("state_id"),
                    m0.get("reason_id"), m0.get("po"), m0.get("packaging_id")
                )
            segments = merged
            ...
            # Option B: reason_id -> state info
            # reasons: dict reason_id -> {..., state_id:int, ...}
            # states:  dict state_id  -> {..., state_code:str, ...}
            ...
            for rid, r in reasons.items():
                ...
                reason_to_state[rid] = {"state_id": sid, "state_code": state_code}
                
            logger.info("STEP-11: reason_to_state -> {}", reason_to_state)
            ...
            # shift_lookup_fn = make_shift_lookup(shifts, clk.site_tz)
